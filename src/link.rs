...
impl Link {
    ...
    pub fn count_loops(&self, smoothing: usize) -> usize {
        ...
        for x in self.crosses.iter() {
            if (smoothing & 1 << i) != 0 {
                s.connect(x.i - 1, x.l - 1);
                s.connect(x.j - 1, x.k - 1);
            } else {
                s.connect(x.i - 1, x.j - 1);
                s.connect(x.k - 1, x.l - 1);
            }
            i = i + 1;
        }
        s.num_components()
    ...
    pub fn jones(&self) -> poly::Polynomial {
        ...
        for i in 0..n {
            ...
            ret = ret +
                  if (height % 2) == 0 {
                // dim - 1 gives the normalized Jones polynomial
                v.pow(dim - 1).shift(height as i32)
            } else {
                -v.pow(dim - 1).shift(height as i32)
            };
        }
        if (self.nm % 2) == 0 { ret } else { -ret }.shift(self.np - 2 * self.nm)
    }
}
